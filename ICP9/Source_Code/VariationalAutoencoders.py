...
#xavier initialization is helpful in figuring correct weights statistically
...
def train(network_architecture, learning_rate=0.005,
          batch_size=100, training_epochs=5, display_step=5):
    ...
    for epoch in range(training_epochs):
        ...
        cost_data.append(avg_cost)
        # Display logs per epoch step
        if epoch % display_step == 0:
            print("Epoch:", '%04d' % (epoch+1), 
                  "cost=", "{:.9f}".format(avg_cost))
    return vae, cost_data
...
training_epochs = 10
#plotting the reconstruction cost
...
vae_2d,new_cost = train(network_architecture, training_epochs=10)
